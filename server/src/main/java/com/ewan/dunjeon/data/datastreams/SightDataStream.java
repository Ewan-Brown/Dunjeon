...
import com.ewan.dunjeon.data.DebugDrawer;
...
import com.ewan.meworking.data.server.data.*;
...
import static com.ewan.dunjeon.server.world.WorldUtils.getAngleDiffInAtan2Domain;

public class SightDataStream extends Datastream<SightDataStream.SightStreamParameters> {
    ...
    public void update(Dunjeon d) {
        ...
        for (int i = 0; i < getSubscribers().size(); i++) {
            ...
            if(params.getTrueSight()){
                ...
                tileVisibilityMap.put(new Vector2( Math.floor(sensorPos.x), Math.floor(sensorPos.y)), Set.of(WorldUtils.Side.values()));
...
//                                throw new RuntimeException("Attempting to do worst case but no cached steps!");
                    ...
                    Optional<WorldUtils.IntersectionData> collidingIntersection = Optional.empty();
                    ...
                    while (true) {
                        ...
                        Optional<WorldUtils.IntersectionData> intersectionDataOpt = WorldUtils.getNextGridIntersect(currentPoint, rayEnd);
                        ...
                        WorldUtils.IntersectionData intersectionData = intersectionDataOpt.get();
                        ...
                        switch(e){
                            ...
                            case ENTERABLE -> {
                                ...
                                for (WorldUtils.Corner corner : intersectionData.getSide().getCorners()) {
                    ...
                    if(!doingWorstCase || forceBestCase){
                        ...
                        if (forceBestCase || didCollideAdjacentToPrevious) {
                            ...
                            WorldUtils.IntersectionData intersection = collidingIntersection.get();
                            Optional<Double> currentChosenAngle = Optional.empty();
                            for (WorldUtils.Corner corner : intersection.getSide().getCorners()) {
...
//                             If we are here then the ray has ended and we have NOT collided
                            ...
                            for (WorldUtils.Side side : WorldUtils.Side.values()) {
