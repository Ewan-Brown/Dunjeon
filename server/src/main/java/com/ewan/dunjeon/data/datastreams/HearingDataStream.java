...
public class HearingDataStream extends Datastream<HearingDataStream.HearingDataStreamParameters> {
    ...
    private final List<SoundRequest> amalgamatedSoundEvents = new ArrayList<>();
    ...
    final int filled_tile_weight = 3;
    final int max_perceived_dist = 20;
    final double max_random = 2;
    @Override
...
//                            vector2 -> (vector2.x >= params.sensorFloor.getWidth()
//                                            || vector2.y >= params.sensorFloor.getHeight()
//                                            || vector2.x < 0
//                                            || vector2.y < 0));
                    double dist = params.hearingSourceLocation.distance(sound.sourceLocation);
                    if(dist < max_perceived_dist) {
                        double distCoeff = 1.0 - (dist * dist)/(max_perceived_dist*max_perceived_dist);
                        float perceivedIntensity = (float)(sound.intensity*distCoeff);
//                        float randomness = (float)(Math.min(tiles.size(),10)/3.0);
                        float randomness = 0.1f;
                        HeardSoundEvent event = new HeardSoundEvent(sound.category, randomizeVector2(sound.sourceLocation, randomness), perceivedIntensity, sound.sourceUUID == subscriber.getParameters().sensorHostUUID, d.getTimestamp());
                        events.add(event);
                    }
